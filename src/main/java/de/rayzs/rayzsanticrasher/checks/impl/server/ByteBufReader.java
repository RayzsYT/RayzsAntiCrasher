		...
		try {
			...
			if (byteBuf.array().length > 6500) {
				...
				if(player != null) {
					...
					api.doNotify(RayzsAntiCrasher.getInstance().getCrashReportMessage(clientAddress, byteBuf.array().length, this.getClass().getSimpleName(), byteBuf.toString()), player);
				}
			}
			list.add(byteBuf.readBytes(byteBuf.readableBytes()));
