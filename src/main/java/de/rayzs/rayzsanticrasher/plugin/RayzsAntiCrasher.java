...
import de.rayzs.rayzsanticrasher.checks.impl.listener.IllegalChat;
...
import de.rayzs.rayzsanticrasher.packet.TinyProtocol;
...
public class RayzsAntiCrasher extends JavaPlugin {
	...
	public void disable() {
		isRunning = false;
		try { getCheckFile().getYAML().
			save(getCheckFile().getFile());
	...
	public void implementsChecks() {
		// Server {
		api.addCheck(new IllegalConnection(), true);
		// }
		...
		api.addCheck(new IllegalChat(), false);
		// }
	}
	...
	protected void loadPropeties() {
		...
		liveAttackMessage = instance.getConfigFile().search("messages.liveattack").getString(
				"&8>> &c&nSERVER IS UNDER ATTACK&8! &7CPS&8: &b%CPS% &8| &7Blocked &7IP&8'&7s&8: &b&l%BLOCKED% &8| &7Banned &7IP&8'&7s&8: &b&l%BANNED% &8<<");
	...
	public String getLiveAttackMessage(Attack attack, Integer CPS, Integer BLOCKED, Integer BANNED) {
		final String text = (liveAttackMessage.replace("%CPS%", CPS.toString())
		.replace("%BLOCKED%", BLOCKED.toString()).replace("%BANNED%", BANNED.toString()));
		return ChatColor.translateAlternateColorCodes('&', text);
	}
