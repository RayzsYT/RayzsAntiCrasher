...
public class RayzsAntiCrasherAPI {
	...
	public Boolean isVPN(String clientAddress) {
		final String vpnKey = instance.getVPNKey();
		final SecuredJsonReader securedJsonReader = new SecuredJsonReader(
				"https://vpnapi.io/api/" + clientAddress + "?key=" + vpnKey + ".json");
		final Boolean vpn = (Boolean) securedJsonReader.get("security", "vpn");
		final Boolean proxy = (Boolean) securedJsonReader.get("security", "proxy");
		final Boolean tor = (Boolean) securedJsonReader.get("security", "tor");
		if (vpn || proxy | tor)
			return true;
		return false;
	}
