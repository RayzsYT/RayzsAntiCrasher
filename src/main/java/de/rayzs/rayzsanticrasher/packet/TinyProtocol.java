...
import de.rayzs.rayzsanticrasher.packet.Reflection.FieldAccessor;
import de.rayzs.rayzsanticrasher.packet.Reflection.MethodInvoker;
...
public class TinyProtocol {
	...
	public void injectPlayer(Player player) {
		final Channel playerChannel = getChannel(player);
		injectChannelInternal(playerChannel).player = player;
	}
		...
		try {
			...
			if (interceptor == null) {
				...
				if (channel.pipeline().get("decompress") != null)
					channel.pipeline().addAfter("decompress", "cf_decompress", interceptor);
				else
					channel.pipeline().addAfter("splitter", "cf_decompress", interceptor);
	...
	private final class PacketInterceptor extends ChannelDuplexHandler {
		...
		public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
			...
			final String clientAddress = channel.remoteAddress().toString().split(":")[0].replace("/", "");
			...
			try {
				...
				if (msg == null) {
					...
					api.ipTable(clientAddress, true);
					return;
				}

				if (api.doIPTable())
					if (api.getServerAttack().isBlacklisted(clientAddress)) {
						api.disconnectChannel(channel);
						api.ipTable(clientAddress, true);
						return;
					}
				 ...
				 if (msg instanceof UnpooledUnsafeDirectByteBuf) {
			            final ByteBuf byteBuf = Unpooled.wrappedBuffer((ByteBuf)msg);
				...
				if (packet instanceof PacketHandshakingInSetProtocol) {
					...
					double packetSize = 0;
					for (byte current : byteBuf.array()) packetSize += Double.parseDouble(current + "");
					api.getAttackPacketCounter().addPacket("attackPacket :3");
					if (packetSize < 1000 || packetSize > 2000) {
						api.disconnectChannel(channel);
						api.getServerAttack().addBlacklist(clientAddress);
						return;
					}
				...
				try {
					final ByteBuf byteBuf = Unpooled.buffer();
					packet.b(new PacketDataSerializer(byteBuf));
					if (byteBufCheck(byteBuf, channel)) {
						api.doNotify(RayzsAntiCrasher.getInstance().getCrashReportMessage(clientAddress,
								byteBuf.array().length, ByteBufReader.class.getSimpleName(), byteBuf.toString()),
								player);
						return;
					}
				} catch (Exception exception) {
				...
				if (!api.existPacketCounter(channel))
					api.createPacketCounter(channel);
				final PacketCounter packetCounter = api.getPacketCounter(channel);
				packetCounter.addPacket(packetName);
				final Integer amount = packetCounter.getPacketAmount(packetName);
				handleLoginStart(channel, msg);
				if (player != null) {
					new Crasher(player, channel, packetName, packet, amount);
					new Crasher(player, channel, packetName, packet);
					msg = onPacketInAsync(player, channel, msg);
				} else
					new Crasher(channel, socketAddress, packetName, packet, msg, amount);
	...
	protected Boolean byteBufCheck(ByteBuf byteBuf, Channel channel) {
		final Integer byteAmount = byteBuf.array().length;
		final Integer readableBytes = byteBuf.readableBytes();
		final Integer capazity = byteBuf.capacity();
		final Integer refCount = byteBuf.refCnt();
